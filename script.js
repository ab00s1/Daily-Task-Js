...
let progText = document.querySelector(
  "body > div > div.progress-bar > div > span"
);
...
document.querySelector("body > div > p:nth-child(2)").innerText =
  allQuotes[completedGoalCount];
document.querySelector("body > div > p:nth-child(7)").innerText =
  bottomQuotes[completedGoalCount];
  ...
  if (count < 5) {
    ...
    checkBoxList[checkBoxList.length - 1].addEventListener("click", (e) => {
      ...
      if (allInputFields) {
        ...
        errorMessage.innerText = `Please set all the ${inputFields.length} goals!`;
